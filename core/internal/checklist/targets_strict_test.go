...
func TestSectionBoundItemsFallBackOnlyToTheirParentSection(t *testing.T) {
	...
	for _, target := range targets {
		...
		case "7.3.1":
			want := []string{courseSectionByTitle("Seguimiento a la Formaci"), courseSectionByTitle(seguimientoSectionTitle)}
		...
		case "7.1.1":
			want := []string{courseSectionByTitle(seguimientoSectionTitle)}
			if !reflect.DeepEqual(target.CSSSelectorFallbacks, want) {
